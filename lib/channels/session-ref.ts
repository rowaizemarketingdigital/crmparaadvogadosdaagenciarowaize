...
export type ChannelSessionRef =
  ...
  | { provider: "instagram"; instagram_business_id: string };
...
export const CHANNEL_SESSION_REF_COLUMNS =
  "provider, uazapi_instance_id, meta_phone_number_id, zernio_account_id, instagram_business_id";
...
export function resolveSessionRef(session: ChannelSessionRef): string {
  switch (session.provider) {
    ...
    // `ig_user_id` da conta conectada — é o que `entry[].id` traz em todo
    // webhook da Graph API do Instagram Login (migration 0237).
    case "instagram":
      return session.instagram_business_id;
  }
}
